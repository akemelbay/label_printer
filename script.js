        ...
        fetch(`https://sheets.googleapis.com/v4/spreadsheets/${SPREADSHEET_ID}/values/${sheetRange}?key=${API_KEY}`)
            ...
            .then(data => {
                ...
                headers.forEach((header, index) => {
                    const isChecked = ['Date', 'Substrate name', 'Material name', 'Substrate T, C', 'DC Power max, %', 'Calibrated Rate, A/s', 'Thickness, nm', 'Deposition rate (av.), A/sec'].includes(header) ? 'checked' : '';
